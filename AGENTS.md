...
- `app/components/` — `site-navigation`, `site-footer`, `contact-section` (client; shared by every route), `page-chrome` (server layout primitives: `SubPage`, `PageHero`, `PageSection`, `CardGrid`, `NumberedSteps`, `CheckList`, `FaqSection`, `RelatedPages`), `site-links` (nav data + cross-route anchor handling), `json-ld`, `motion-variants`
